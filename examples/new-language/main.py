...
# ## Adding some tests
#
# The first thing you might want to do is to add a test to check that your implementation is working correctly. For example you could try to simply load and solve the Lorenz attractor using the new Jax code generator. That code could look as follows
...
# ```
#
# There are two classes in this module; the code printer `JaxPrinter` and the code generator `JaxCodeGenerator`
#
...
#
# ### Code generator
# The class `JaxCodeGenerator` is a subclass of the abstract base class [`CodeGenerator`](https://github.com/finsberg/gotranx/blob/57b0c1c80555503e5a7a2729aa57a3660a4b7db0/src/gotranx/codegen/base.py#L95) which implements functionality to generate all the methods we need. For example we need one method to generate code for the right hand side (i.e `def rhs(t, states, parameters)`), and another method to generate code for initializing the default parameter values. For both of these methods we can use the same printer to convert the sympy expressions into code, but we need other types of logic to tell what should be in the different methods.
#
# We want the class `JaxCodeGenerator` to change three things compared to `PythonCodeGenerator`. First we want to set the code printer to be the `JaxPrinter` that we just discussed. Next we want to add the imports that we need at the top (we also enable float64 since arrays are [by default float32 in Jax](https://jax.readthedocs.io/en/latest/notebooks/Common_Gotchas_in_JAX.html#double-64bit-precision))
#
# Finally we set the template to be our custom template which we will implement in a second module called `template.py` (in the next section). This module implements the template for the different methods that we want to generate.
#
#
# ## Adding a new template
# Now we will implement the template in `template.py`, which needs to follow the {py:class}`Template protocol<gotranx.templates.Template>`. In other words we need to implement all the methods that are part of the `Template` class. Our template will be pretty close to the {py:mod}`Python template<gotranx.templates.python>`
#
#
# For `state_index`, `parameter_index`, `monitor_index` and `missing_index` we will just use the same methods as have been implemented in the Python template so we will just import those directly. We will also import the `acc` function that is used in the `init_state_values`
#
# ```python
...
# ```
#
# Finally for the `method` function we just need to make sure to create the correct return array, which now should be an array containing the values `_values_0`, `_values_1`, and so on.
#
# ```python
# def method(
#     name,
